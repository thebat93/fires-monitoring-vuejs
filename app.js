Vue.component("filter-panel", {
  ...
  computed: {
    isDisabled: function() {
      if (this.calendars !== undefined) {
        return (
          this.calendars[0].config.maxDate.toString() ==
          this.calendars[0].selectedDates[0].toString()
        );
      } else return false;
    }
  },
  methods: {
    ...
    updateDate: function(e) {
      this.filters.date = [
        `${this.calendars[0].formatDate(
          this.calendars[0].selectedDates[0],
          "Y-m-d"
        )}T${this.timePickers[0].formatDate(
          this.timePickers[0].selectedDates[0],
          "H:i:S"
        )}.000Z`,
        `${this.calendars[0].formatDate(
          this.calendars[0].selectedDates[0],
          "Y-m-d"
        )}T${this.timePickers[1].formatDate(
          this.timePickers[1].selectedDates[0],
          "H:i:S"
        )}.000Z`
      ];
      this.updateFilter();
    ...
    updateDates: function(e) {
      this.filters.date = [
        `${this.calendars[1].formatDate(
          this.calendars[1].selectedDates[0],
          "Y-m-d"
        )}T${this.timePickers[2].formatDate(
          this.timePickers[2].selectedDates[0],
          "H:i:S"
        )}.000Z`,
        `${this.calendars[2].formatDate(
          this.calendars[2].selectedDates[0],
          "Y-m-d"
        )}T${this.timePickers[3].formatDate(
          this.timePickers[3].selectedDates[0],
          "H:i:S"
        )}.000Z`
      ];
      this.updateFilter();
...
var app = new Vue({
  ...
  data: {
    ...
    openPanel: false,
    url:
      "https://fire-rs.gazprom-spacesystems.ru/proxy_gazcom_rs/geoserver/GSS/wms",
    baseLayers: {
      Bing: new L.BingLayer(
    ...
    updateFilter: function(filtersObject) {
      ...
      for (let prop in this.userFilters) {
        switch (prop) {
          case "date":
            filterArray.push(
              `(date BETWEEN ${this.userFilters[prop][0]} AND ${
                this.userFilters[prop][1]
              })`
            );
            this.threatsFilterString = `(date BETWEEN ${
              this.userFilters[prop][0]
            } AND ${this.userFilters[prop][1]})`;
            break;
          case "area":
          ...
          case "confidence_user":
            filterArray.push(
              `(confidence_user IN (${this.userFilters[prop].map(
                index =>
                  `'${this.filtersData.confidences[index].toLowerCase()}'`
              )}))`
            );
            break;
          case "satellite":
            filterArray.push(
              `(satellite IN (${this.userFilters[prop].map(
                index => `'${this.filtersData.satellites[index]}'`
              )}))`
            );
            break;
          case "source":
            filterArray.push(
              `(source IN (${this.userFilters[prop].map(
                index => `'${this.filtersData.sources[index]}'`
              )}))`
            );
        }
      }
  ...
  mounted: function() {
    ...
    L.control
      .groupedLayers(this.baseLayers, this.groupedOverlays, {
        groupCheckboxes: true
      })
      .addTo(this.map);
    const layerSwitcher = document.querySelector(
